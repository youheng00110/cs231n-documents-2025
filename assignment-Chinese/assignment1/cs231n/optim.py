...
def rmsprop(w, dw, config=None):
    ...
    # config['cache']中的缓存值。                                                #
    #############################################################################
    vdw= config["decay_rate"] * config["cache"] + (1 - config["decay_rate"]) * (dw ** 2)
    next_w=w-config["learning_rate"]*dw/(np.sqrt(vdw)+config["epsilon"])
    config["cache"] = vdw  # 更新缓存，供下次迭代使用
    ###########################################################################
    #                             你的代码结束                                #
    ###########################################################################
...
def adam(w, dw, config=None):
    ...
    ###########################################################################
    config["t"] += 1
    m = config['beta1'] * config['m'] + (1 - config['beta1']) * dw           # 一阶矩
    v = config['beta2'] * config['v'] + (1 - config['beta2']) * (dw ** 2)    # 二阶矩
    m_corrected = m / (1 - config['beta1'] ** config['t'])                   # 偏差修正
    v_corrected = v / (1 - config['beta2'] ** config['t'])                   # 偏差修正
